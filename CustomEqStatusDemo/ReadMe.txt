================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : CustomEqStatusDemo プロジェクトの概要
===============================================================================

この CustomEqStatusDemo アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、CustomEqStatusDemo アプリケーションを構成する各ファイルの内容の概要が含まれています。

CustomEqStatusDemo.vcxproj
...
CustomEqStatusDemo.vcxproj.filters
...
CustomEqStatusDemo.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CCustomEqStatusDemoApp アプリケーション クラスを宣言します。

CustomEqStatusDemo.cpp
    これはメインのアプリケーション ソース ファイルで、CCustomEqStatusDemoApp アプリケーション クラスが含まれています。

CustomEqStatusDemo.rc
...
res\CustomEqStatusDemo.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル CustomEqStatusDemo.rc にインクルードされます。

res\CustomEqStatusDemo.rc2
...
CustomEqStatusDemoDlg.h, CustomEqStatusDemoDlg.cpp - ダイアログ
    これらのファイルには、CCustomEqStatusDemoDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる CustomEqStatusDemo.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、CustomEqStatusDemo.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

CustomEqStatusDemo.manifest
