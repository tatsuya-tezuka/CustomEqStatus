================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : CustomCsvToXml プロジェクトの概要
===============================================================================

この CustomCsvToXml アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、CustomCsvToXml アプリケーションを構成する各ファイルの内容の概要が含まれています。

CustomCsvToXml.vcxproj
...
CustomCsvToXml.vcxproj.filters
...
CustomCsvToXml.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CCustomCsvToXmlApp アプリケーション クラスを宣言します。

CustomCsvToXml.cpp
    これはメインのアプリケーション ソース ファイルで、CCustomCsvToXmlApp アプリケーション クラスが含まれています。
...
res\CustomCsvToXml.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル CustomCsvToXml.rc にインクルードされます。

res\CustomCsvToXml.rc2
...
CustomCsvToXmlDlg.h, CustomCsvToXmlDlg.cpp - ダイアログ
    これらのファイルには、CCustomCsvToXmlDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる CustomCsvToXml.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、CustomCsvToXml.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

CustomCsvToXml.manifest
